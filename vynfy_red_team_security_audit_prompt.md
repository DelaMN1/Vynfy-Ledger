# Codex Red-Team Security Audit Prompt for Vynfy Ledger

You are a senior security engineer and red-team specialist tasked with performing a comprehensive, adversarial security audit of this codebase, system design, and application.
...
Check for, but do not limit yourself to, the following:
...
Codex must use this prompt as an operational checklist, not a passive reading document.

Codex must:
1. Review the entire codebase and architecture
2. Enumerate trust boundaries and attacker entry points
3. Inspect frontend, backend, auth, persistence, and deployment assumptions
4. Identify both direct vulnerabilities and exploit chains
5. Flag confirmed vulnerabilities and plausible risks separately where helpful
6. Recommend concrete remediations
7. Prefer precision, depth, and adversarial thinking over generic advice

The audit is incomplete if it only checks common OWASP patterns without looking for application-specific logic flaws.
