...
def _active_categories(transaction_type: str) -> list[tuple[int, str]]:
    items = Category.query.filter_by(type=transaction_type, is_active=True).order_by(Category.name.asc()).all()
    return [(item.id, item.name) for item in items]


def _active_accounts() -> list[tuple[int, str]]:
    items = Account.query.filter_by(is_active=True).order_by(Account.name.asc()).all()
    return [(item.id, item.name) for item in items]


def _active_payment_methods() -> list[tuple[int, str]]:
    items = PaymentMethod.query.filter_by(is_active=True).order_by(PaymentMethod.name.asc()).all()
    return [(0, "Select payment method")] + [(item.id, item.name) for item in items]


def assign_form_choices(form, transaction_type: str) -> None:
...
def assign_filter_choices(form, *, user: User, transaction_type: str | None = None) -> None:
    ...
    status_values = {status.value for status in RevenueStatus} | {status.value for status in ExpenseStatus}
    if transaction_type == TransactionType.REVENUE.value:
        status_values = {status.value for status in RevenueStatus}
    if transaction_type == TransactionType.EXPENSE.value:
        status_values = {status.value for status in ExpenseStatus}
    form.status.choices = [("", "All statuses")] + [(value, value) for value in sorted(status_values)]


def visible_transactions_query(user: User, transaction_type: str | None = None):
...
def apply_filters(query, filters: TransactionFilters):
    if filters.q:
...
def list_transactions(*, user: User, transaction_type: str | None = None, filters: TransactionFilters | None = None):
...
def _can_edit(item: Transaction, actor: User) -> bool:
    ...
    if item.transaction_type == TransactionType.EXPENSE.value:
        return item.status in {ExpenseStatus.DRAFT.value, ExpenseStatus.RETURNED.value}
...
def create_transaction_from_form(*, form, transaction_type: str, actor: User) -> Transaction:
...
def update_transaction_from_form(*, transaction: Transaction, form, actor: User) -> Transaction:
